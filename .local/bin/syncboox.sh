...
PHONE_DIR="/run/user/1000/gvfs/mtp:host=ONYX_BENGALP-IOT-IDP__SN%3A32ED0F70_9CFCCC09/Internal shared storage/Books"
PC_DIR="/run/user/1000/gvfs/smb-share:server=truenas.local,share=data/media/books"
export TMPDIR=/tmp
...
rsync -ahv --ignore-existing --progress --no-times --no-perms --no-group --temp-dir="$TMPDIR" "$PC_DIR" "$PHONE_DIR"
